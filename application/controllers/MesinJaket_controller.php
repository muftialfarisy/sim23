...
class MesinJaket_controller extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        //     redirect('/');
        // }
        $this->load->model('MesinJaket_model');
    }

    public function index()
    {
        $this->load->view('Mesin/mesinJaket');
    }
    public function read()
    {
        $jabatan = $this->session->userdata('jabatan');
        header('Content-type: application/json');
        if ($this->MesinJaket_model->read()->num_rows() > 0) {
            // var_dump($this->user_model->read()->result());
            foreach ($this->MesinJaket_model->read()->result() as $mesin) {
                $data[] = array(
                    'id' => $mesin->id,
    ...
    public function add()
    {
        ...
        if (($this->MesinJaket_model->create($data)) && ($this->MesinJaket_model->create_rata($rata))) {
            echo json_encode('sukses');
        }
        // $this->MesinJaket_model->create($data);
        // echo json_encode('sukses');
    }
    public function delete()
    {
        $id = $this->input->post('id');
        if ($this->MesinJaket_model->delete($id)) {
            echo json_encode('sukses');
        }
    }
    ...
    public function edit()
    {
        ...
        if ($this->MesinJaket_model->update($id, $data)) {
            echo json_encode('sukses');
        }
    }
    ...
    {
        $id = $this->input->post('id');
        $user = $this->MesinJaket_model->getMesin($id);
        if ($user->row()) {
    ...
    public function get_rata()
    {
        $id = $this->input->post('id');
        $no = 9;
        $user = $this->MesinJaket_model->getRata($no);
        if ($user->row()) {
            echo json_encode($user->row());
        }
    }
